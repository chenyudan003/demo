...
h = 1e-9
b = np.pi/a
kk = np.arange(b*1e-7,b+0.01,b/40)
...
for k in kk:
    print('k='+str(k))
    a = switch2(1e+4, 2e+4, 12e+7, 1e+2, 1)
    try:
        result0.append(a[0])
...
ltime = str(localtime)
result.to_excel('./data/PWE(flex)%s [H=%s, n=%s, l1=%s, l2=%s].xlsx' % (ltime, h, nrsquare, l1, l2), index=False)
...
plt.title('PWE(flex) [H=%s, n=%s, l1=%s, l2=%s]' % (h, nrsquare, l1, l2))
plt.grid()
plt.savefig('./fig/PWE(flex) [H=%s, n=%s, l1=%s, l2=%s].png' % (h, nrsquare, l1, l2))
plt.show()
